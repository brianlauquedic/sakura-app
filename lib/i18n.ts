...
export const translations = {
  ...
  // ── Mutual Pool (MutualPool.tsx) ─────────────────────────────────
  mutualBrandTag:       { zh: "SAKURA MUTUAL v0.2 · 數學結算保險", en: "SAKURA MUTUAL v0.2 · ZK-SETTLED INSURANCE", ja: "SAKURA MUTUAL v0.2 · ZK決済保険" },
  mutualHeadline1:      { zh: "理賠由", en: "Claims are settled by", ja: "請求は" },
  mutualHeadlineMath:   { zh: "數學", en: "math", ja: "数学" },
  mutualHeadline2:      { zh: "結算，而非信任。", en: ", not trust.", ja: "で決済される、信頼ではなく。" },
  mutualDescription:    { zh: "質押 USDC，為您的 Kamino 倉位綁定 Poseidon 承諾；當健康因子跌破您設定的觸發閾值，Groth16 證明自動釋放救援款項。無人工審批、無裁量權、無對手方風險——僅靠鏈上", en: "Stake USDC, lock a Poseidon commitment to your Kamino position, and a Groth16 proof automatically releases a rescue payout when your health factor drifts below your chosen trigger. No human approvals, no discretion, no counter-party risk — just on-chain", ja: "USDCをステークし、KaminoポジションにPoseidonコミットメントを紐づけ、ヘルスファクターが指定のトリガーを下回るとGroth16証明が自動で救済支払いを解放します。人間の承認なし、裁量なし、カウンターパーティリスクなし — ただオンチェーンの" },
  mutualDescriptionEnd: { zh: "配對驗證。", en: ".", ja: "ペアリングのみ。" },
  mutualPoolState:      { zh: "互助池狀態", en: "POOL STATE", ja: "プール状態" },
  mutualPoolNotInitDemo:{ zh: "互助池尚未在 devnet 初始化（demo 模式）。", en: "Pool not yet initialized on devnet (demo mode).", ja: "devnetでプール未初期化（デモモード）。" },
  mutualPoolNotInit:    { zh: "互助池尚未初始化。請執行 `scripts/initialize-insurance-pool.ts`。", en: "Pool not yet initialized. Run `scripts/initialize-insurance-pool.ts`.", ja: "プール未初期化。`scripts/initialize-insurance-pool.ts` を実行してください。" },
  ...
  mutualRowCoverageOut: { zh: "未償保額", en: "Coverage Outstanding", ja: "未決済カバー" },
  mutualRowClaimsPaid:  { zh: "已賠付", en: "Claims Paid", ja: "支払い済み請求" },
  mutualRowPremiumFee:  { zh: "保費 / 平台費", en: "Premium / Platform Fee", ja: "保険料 / プラットフォーム手数料" },
  ...
  mutualYourPolicy:     { zh: "您的保單", en: "YOUR POLICY", ja: "あなたのポリシー" },
  mutualNoPolicy:       { zh: "您尚無有效保單。購買一份以獲得保障。", en: "You don't have an active policy. Buy one to be protected.", ja: "有効なポリシーがありません。購入して保護を受けてください。" },
  mutualConnectForPolicy:{ zh: "連接錢包以查看您的保單。", en: "Connect a wallet to see your policy.", ja: "ポリシーを表示するにはウォレットを接続してください。" },
  mutualRowActive:      { zh: "有效", en: "Active", ja: "有効" },
  mutualRowCoverageCap: { zh: "保額上限", en: "Coverage Cap", ja: "カバー上限" },
  mutualRowRemaining:   { zh: "剩餘保額", en: "Remaining Coverage", ja: "残りカバー" },
  mutualRowStake:       { zh: "質押（可退還）", en: "Stake (refundable)", ja: "ステーク（返金可）" },
  mutualRowPremiumPaid: { zh: "已付保費", en: "Premium Paid", ja: "支払済保険料" },
  ...
  mutualBuyPolicy:      { zh: "購買保單", en: "BUY A POLICY", ja: "ポリシーを購入" },
  mutualCoverageCap:    { zh: "保額上限", en: "Coverage cap", ja: "カバー上限" },
  mutualPremiumLine:    { zh: "保費 ${p} / 月 · 質押 ${s}（可退）· 等待期 {w}s", en: "premium ${p} / month · stake ${s} (refundable) · waiting period {w}s", ja: "保険料 ${p} / 月 · ステーク ${s}（返金可）· 待機期間 {w}s" },
  mutualBtnSigning:     { zh: "簽名中…", en: "Signing…", ja: "署名中…" },
  mutualBtnBuy:         { zh: "＋ 購買保單", en: "＋ Buy Policy", ja: "＋ ポリシー購入" },
  ...
  mutualBoughtOk:       { zh: "✓ 保單已購買", en: "✓ Policy bought", ja: "✓ ポリシー購入完了" },
  mutualNonceSaved:     { zh: "Nonce 已儲存至 localStorage — 未來 ZK 理賠必備。", en: "Nonce saved to localStorage — required for future ZK claims.", ja: "NonceをlocalStorageに保存 — 今後のZK請求に必須。" },
