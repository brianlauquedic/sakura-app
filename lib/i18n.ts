...
export const translations = {
  ...
  ceilingBody: {
    zh: "$4.48B · Solana DeFi 中，Sakura 12 道 CPI 閘門此刻可觸達的 TVL\n  └ $1.67B  Kamino（借貸）\n  └ $880M  Jupiter（Swap + Lend）\n  └ $920M  Jito（LST）\n  └ $1.01B  Raydium（AMM）\n  ─────────────────────────────────\n  其中 $1.62B 是借貸方的未償債務——AI 代理手握 session key，市況急變的一夜能動到的最痛倉位。\n\n這不是 TAM 推演、不是 4 家錢包 × 0.1% 費率 × 若干年 ARR。這是此刻、這 4 家協議、這 12 道適配器閘住的、可觸達的真數字。數字源 DefiLlama，可 reproduce：`npx tsx scripts/som-analysis/day1-som.ts`。",
    en: "$4.48B · Solana DeFi TVL reachable today through Sakura's 12 CPI cells\n  └ $1.67B  Kamino (lending)\n  └ $880M  Jupiter (Swap + Lend)\n  └ $920M  Jito (LST)\n  └ $1.01B  Raydium (AMM)\n  ─────────────────────────────────\n  Of which $1.62B is outstanding borrow debt — the exact account shape an AI agent with a session key can drain in a single market turn.\n\nThis is not TAM, not four wallets × 0.1% × hypothetical ARR. This is right-now, these four protocols, these twelve adapters — a real number you can verify yourself: `npx tsx scripts/som-analysis/day1-som.ts`. Source: DefiLlama.",
    ja: "$4.48B · Sakura の 12 の CPI ゲートが、今この瞬間に到達可能な Solana DeFi の TVL\n  └ $1.67B  Kamino（レンディング）\n  └ $880M  Jupiter（Swap + Lend）\n  └ $920M  Jito（LST）\n  └ $1.01B  Raydium（AMM）\n  ─────────────────────────────────\n  うち $1.62B が借入側の未返済債務——セッションキーを握った AI エージェントが、市況急変の一夜で引き出せる、最も痛い口座規模。\n\nこれは TAM の推計ではない。4 大ウォレット × 0.1% × 仮想 ARR でもない。今この瞬間、この 4 プロトコル、この 12 のアダプタが閘門となる、到達可能な実数である。出典は DefiLlama、再実行は `npx tsx scripts/som-analysis/day1-som.ts` で。",
  },
